  ...
  try {
    ...
    for (const field of requiredFields) {
      if (!sourceConfig[field]) {
        throw new Error(
          `Missing source database ${field}. Set SOURCE_DB_${field.toUpperCase()} or use --source-env`
        );
      }
      if (!targetConfig[field]) {
        throw new Error(
          `Missing target database ${field}. Set TARGET_DB_${field.toUpperCase()} or use --target-env`
        );
      }
    }
    ...
    console.log(
      `📤 Source: ${sourceConfig.user}@${sourceConfig.host}:${sourceConfig.port}/${sourceConfig.database}`
    );
    console.log(
      `📥 Target: ${targetConfig.user}@${targetConfig.host}:${targetConfig.port}/${targetConfig.database}`
    );
