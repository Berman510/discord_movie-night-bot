...
async function showSetupMenuWithMessage(interaction, currentConfig = null, successMessage = null) {
    ...
    .addFields(
      {
        ...
        value: currentConfig.movie_channel_id
          ? `<#${currentConfig.movie_channel_id}>`
          : 'Not configured',
        inline: true,
      },
      {
        name: `${currentConfig.admin_channel_id ? '✅' : '❌'} Admin Channel`,
        value: currentConfig.admin_channel_id
          ? `<#${currentConfig.admin_channel_id}>`
          : 'Not configured',
        inline: true,
      },
      {
        name: `${currentConfig.watch_party_channel_id ? '✅' : '❌'} Watch Party Channel`,
        value: currentConfig.watch_party_channel_id
          ? `<#${currentConfig.watch_party_channel_id}>`
          : 'Not configured',
        inline: true,
      },
      {
        ...
        value:
          currentConfig.admin_roles?.length > 0
            ? `${currentConfig.admin_roles.length} role(s)`
            : 'Not configured',
        inline: true,
      },
      {
        ...
        value:
          currentConfig.moderator_roles?.length > 0
            ? `${currentConfig.moderator_roles.length} role(s)`
            : 'Not configured',
        inline: true,
      },
      {
        ...
        value:
          currentConfig.voting_roles?.length > 0
            ? `${currentConfig.voting_roles.length} role(s)`
            : 'Not configured',
        inline: true,
      }
    );
  ...
  const menuButtons2 = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId('setup_admin_roles')
      .setLabel('👑 Admin Roles')
      .setStyle(
        currentConfig.admin_roles?.length > 0 ? ButtonStyle.Success : ButtonStyle.Secondary
      ),
    new ButtonBuilder()
      .setCustomId('setup_moderator_roles')
      .setLabel('🛡️ Moderator Roles')
      .setStyle(
        currentConfig.moderator_roles?.length > 0 ? ButtonStyle.Success : ButtonStyle.Secondary
      ),
    new ButtonBuilder()
      .setCustomId('setup_voting_roles')
      .setLabel('👥 Voting Roles')
      .setStyle(
        currentConfig.voting_roles?.length > 0 ? ButtonStyle.Success : ButtonStyle.Secondary
      )
  );

  const menuButtons3 = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
